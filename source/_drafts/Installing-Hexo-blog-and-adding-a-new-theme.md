...
The previous sentence is a cite taken from [this](http://nilclass.com/courses/what-is-a-static-website/#1) page, where you can see a nice animation, that visually presents differences between static and dynamic websites.
...
Assuming you have installed [Node](https://nodejs.org/en/) and [Git](https://git-scm.com/), you can install Hexo Framework (*'hexo-cli'* - Hexo Command Line Interface) by running following line in a command terminal:

``` bash
npm install -g hexo-cli
```

Wait for the installation process to finish, and if everything went well, you can confirm Hexo is installed properly by asking about Hexo version info:

``` bash
hexo --version
```

You should get a list of version informations displayed.

Now, create a new folder where your blog files will reside. We will set this folder to be a *GitHub* repository later.
...
With running ```hexo init``` command, the Hexo framework downloads all the files and folders needed for an initial blog structure. 
...
After it's done, you should see the following files and folders in your blog folder:

{% asset_img "hexo_initial.png" "Hexo folder structure" %}


### Serving Hexo Blog ###
At this moment you already have an empty blog created. To see this newly generated blog, run Hexo command for serving blog locally:

``` bash
hexo server
```

Check how your blog looks like by going to [localhost:4000](http://localhost:4000) address in your browser. By default, the Hexo blog comes with the theme called *[landscape](https://github.com/hexojs/hexo-theme-landscape)*. 

The theme defines *the look and feel* of your site. You can have more than one theme installed, but you need to set which theme is active in the *_config.yml* file, that is in the root(blog) folder. Every theme you have installed is placed in a separate folder, underneath the *'themes'* folder. 
...
Hexo has a great community and there are many different Hexo themes that you can check and try on [Hexo themes](https://hexo.io/themes/index.html) site. In our posts, we will be installing and configuring *[clean-blog](https://github.com/klugjo/hexo-theme-clean-blog)* theme. I have chosen this theme because it is simple to use, but still contains the main features needed for a typical blog.


### Installing a new theme ###
If your local server is still running, press 'CTRL+C' in command prompt for stopping it. We are going to install *[clean-blog](https://github.com/klugjo/hexo-theme-clean-blog)* theme, and delete default *landscape* theme. If you open *'themes'* folder in Windows Explorer, you will see *landscape* folder as the only folder there. For installing *[clean-blog](https://github.com/klugjo/hexo-theme-clean-blog)* theme, enter:
...
At this moment, we have deleted the default theme, installed a new theme, but we still need one more step to activate the newly installed theme. In editor of your choice ([Visual Studio Code](https://code.visualstudio.com/), [Atom](https://atom.io/), [Sublime](https://www.sublimetext.com/) ...) open your blog folder, then open *_config.yml* that is under your root(blog) folder. 
...
Again, start Hexo server locally (```hexo server```), and check how your site looks now, with the new theme applied: [localhost:4000](http://localhost:4000).
...
If you are not, there is a step by step process described [here](https://help.github.com/en/github/importing-your-projects-to-github/adding-an-existing-project-to-github-using-the-command-line).
